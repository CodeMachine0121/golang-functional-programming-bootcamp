...
一個為期 30 天的實作型 Bootcamp，帶你以 Go 實踐函數式程式設計（FP）。從語言哲學、泛型、不可變性、Functor/Applicative/Monad，到真實世界的架構與效能，循序漸進、理論搭配實作。
...
```
.
├── Day 1 - Golang 哲學與 Functional Programming 純粹主義的碰撞.md
├── Day 2 - 超越一等公民-高階函數的組合與 Currying 的模擬.md
├── Day 3 - 泛型- 啟類型安全 Functional Programming 的鑰匙.md
├── readme.md  ← 本文件
└── codes/
    ├── day2/
    │   ├── go.mod
...
- 根目錄的 Day X 檔：每一天的理論與筆記。
- `codes/dayX/`：對應每日重點的程式碼樣例與練習。
...
- Go 1.18 以上（建議 1.22+）
- 作業系統：Windows/macOS/Linux（本說明以 Windows PowerShell 為例）
- Git（選用）
...
如要自行新增某天的範例，建議在 `codes/dayN` 建立獨立 `go.mod`，以利實驗與依賴管理。

---

## 學習方式建議

- 先閱讀對應的 Day X 筆記，再進入 `codes/dayX` 動手實作。
- 嘗試以測試或小範例重現文中概念（map/filter/reduce、Either、State、Reader 等）。
- 寫下你對「何時維持純粹性、何時務實妥協」的思考與取捨。
...
- 將 `Either` 與 `Reader` 組合成 `ReaderEither` 以同時處理環境與錯誤。
- 為 `[]T` 與 `chan T` 各自實作 `Map/Filter/Reduce` 並做基準測試（`go test -bench .`）。
- 將遞迴演算法以 trampolining 轉為迭代，並比較最大深度與記憶體佔用。

---

## 貢獻與回饋

- 歡迎提交 Issue/PR：新增日次練習、修正文稿、補充測試或改進 API 設計。
- 任何建議（教材順序、難度、實務案例）都非常歡迎。

---

## 參考資源

- The Go Programming Language（官方文件）：https://go.dev/doc/
- Go Generics 指南與設計提案：
  - https://go.dev/doc/tutorial/generics
  - https://go.dev/blog/intro-generics
- Property-based testing：
  - gopter（或 fastcheck 類似觀念）: https://github.com/leanovate/gopter
- 持久性資料結構參考：
